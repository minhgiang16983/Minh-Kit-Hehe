...
import (
	...
	kitlogger "github.com/minhgiang16983/Minh-Kit-Hehe/logger"
	"go.uber.org/zap"
...
func NewLogger(logLevel gormlogger.LogLevel) Logger {
	return Logger{
		LogLevel:                  logLevel,
...
func (l Logger) logger(ctx context.Context) *zap.Logger {
	logger := kitlogger.GetLogger(ctx).(*kitlogger.Logger).Logger
	if l.Context != nil {
